...
def get_image_browse_urls(user=None):
    ...
    for filename in get_image_files(user=user):
        src = utils.get_media_url(filename.split('media')[1])
        if getattr(settings, 'CKEDITOR_IMAGE_BACKEND', None):
            thumb = utils.get_media_url(utils.get_thumb_filename(filename.split('media')[1]))
        else:
            thumb = src
        images.append({
            'thumb': thumb,
